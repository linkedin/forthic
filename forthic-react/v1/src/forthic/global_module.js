...
class GlobalModule extends Module {
    ...
    word_Element(interp) {
        ...
        const clean_element_name = element_name.replace(/[()[\]<>~!@#$%^&+=\-*\s]/g, "")
        ...
        function Result() {
            ...
            let res = React.createElement(element_class, Result.props, ...rendered_content)
            return res
        }
        interp.stack_push(Result)
    }
    ...
    async word_SERVER_INTERPRET(interp) {
        ...
        await axios.post(get_forthic_route(), {
            ...
            .catch(function (error) {
                if (window.exceptionHandler) {
                    window.exceptionHandler({type: "SERVER-INTERPRET", forthic: word, args: args, error: error.response.data})
                }
                else {
                    console.log(error);
