...
from tests.tests_py.v3.modules.jira_context import JiraTestContext
...
class TestJiraModule(unittest.TestCase):
    ...
    def test_TIME_IN_STATE_timestamps(self):
        field = "status"
        resolution = "Fixed"

        # NOTE: The following data would come from something like `'PROJ-1234' ['status'] jira.CHANGELOG`
        changes = [
            {"date": 1626830097, "field": "status", "from": "", "to": "Open"},
            {"date": 1629687367, "field": "status", "from": "Open", "to": "Scoping", "from_": "1", "to_": "10128"},
            {"date": 1632772419, "field": "status", "from": "Scoping", "to": "In Development", "from_": "10128", "to_": "10194"},
            {"date": 1636014965, "field": "status", "from": "In Development", "to": "Closed", "from_": "10194", "to_": "6"}
        ]
